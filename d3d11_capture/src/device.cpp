...
Device::Device(CComPtr<IDXGIAdapter1> dxgi_adapter1) : dxgi_adapter1_{std::move(dxgi_adapter1)} {
  ...
  constexpr std::array<D3D_FEATURE_LEVEL, 3> feature_levels = {D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0,
                                                               D3D_FEATURE_LEVEL_10_1};

  const HRESULT hr =
      D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, feature_levels.data(), feature_levels.size(),
                        D3D11_SDK_VERSION, &d3d11_device_, &d3d_feature_level_, &d3d11_device_context_);
  if (FAILED(hr)) {
...
uint32_t Device::DedicatedVideoMemory() const {
