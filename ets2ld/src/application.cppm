...
class Application {
 ...
 private:
  ...
  void LaneDetectorInitializerThread(capture::Settings capture_settings, ufld::Settings ufld_settings);
  void LaneDetectorThread(capture::Settings capture_settings);

  void HandleLaneDetectionEnableChanged(bool enable, capture::Settings capture_settings, ufld::Settings ufld_settings);
};
...
void ets2ld::Application::Run() {
  ui_.SetOnLaneDetectionEnableChanged(std::bind(&Application::HandleLaneDetectionEnableChanged, this,
...
void ets2ld::Application::LaneDetectorInitializerThread(capture::Settings capture_settings,
  ...
  try {
    ...
    lane_detection_thread_ = std::thread{&Application::LaneDetectorThread, this, capture_settings};
...
void ets2ld::Application::LaneDetectorThread(capture::Settings capture_settings) {
  ...
  while (!stop_lane_detection_signal_) {
    ...
    ufld::LaneDetectionResult result = lane_detector_->Detect(frame, frame.clone());
    {
      std::lock_guard<std::mutex> lock{lane_detection_mutex_};
...
[[nodiscard]] bool AreModelSettingsEqual(const ufld::Settings& ufld_settings,
...
void ets2ld::Application::HandleLaneDetectionEnableChanged(bool enable, capture::Settings capture_settings,
                                                           ufld::Settings ufld_settings) {
  ...
  if (enable) {
    if (lane_detector_ != nullptr && AreModelSettingsEqual(ufld_settings_, lane_detector_.get())) {
      lane_detection_thread_ = std::thread{&Application::LaneDetectorThread, this, capture_settings_};
    } else {
      std::thread{&Application::LaneDetectorInitializerThread, this, capture_settings_, ufld_settings_}.detach();
    }
  } else {
    stop_lane_detection_signal_ = true;
    if (lane_detection_thread_.joinable())
